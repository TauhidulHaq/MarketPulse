  ...
  const fetchProducts = async () => {
    try {
      const res = await axios.get(`/api/products`, {
        params: { shopId, revenueFilter: filters.revenue, stockFilter: filters.stock, performanceFilter: filters.performance }
      });
      ...
      <main className="flex-1 ml-56 p-8">
        ...
        <div className="card px-5 py-4 mb-8 flex items-center gap-4 animate-fade-in">
          ...
          <select className="filter-select" onChange={(e) => setFilters({...filters, revenue: e.target.value})}>
            <option value="">Revenue</option><option value="highest">Highest</option><option value="lowest">Lowest</option>
          </select>
          <select className="filter-select" onChange={(e) => setFilters({...filters, stock: e.target.value})}>
            <option value="">Stock Level</option><option value="high">High</option><option value="moderate">Moderate</option><option value="low">Low</option>
          </select>
          
          <select className="filter-select" onChange={(e) => setFilters({...filters, performance: e.target.value})}>
            <option value="">Performance</option><option value="highest">Highest</option><option value="lowest">Lowest</option>
          </select>
          ...
          {products.map((product) => (
            <div key={product._id} className="animate-slide-up">
              ...
              <div className="bg-gray-200/50 p-6 rounded-[2.5rem] grid grid-cols-4 gap-6 items-center">
                ...
                <SummaryCard title="Total Revenue" value={`$${product.revenue?.toLocaleString()}`} change={product.revenueTrend} />
                <SummaryCard title="Stock Level" value={product.stock > 50 ? 'High' : (product.stock > 10 ? 'Moderate' : 'Low')} />
                <SummaryCard title="Performance" value={`${product.performance}%`} change={product.performanceTrend} />
              </div>
            </div>
          ))}
        </div>
