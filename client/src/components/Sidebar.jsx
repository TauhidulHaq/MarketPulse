...
const Sidebar = ({ shopName, shopColor }) => {
  ...
  const navItems = [
    ...
    { label: 'Update Product Price', path: `/shop/${shopId}/update-price`, icon: 'inventory' },
    { label: 'Sales Chart', path: `/shop/${shopId}/sales-chart`, icon: 'cart' },
  ...
  return (
    <aside className="w-56 min-h-screen bg-white border-r border-gray-100 flex flex-col fixed left-0 top-0 z-40">
      <div className="px-5 pt-5 pb-4 border-b border-gray-100">
        <div className="flex items-center gap-3">
          <div
            ...
            style={{ backgroundColor: shopColor || '#8B7A2E' }}
          >
            {shopName?.charAt(0) || 'S'}
          </div>
          <div>
            <h2 className="text-sm font-bold text-primary-600 leading-tight">{shopName || 'Shop'}</h2>
            <p className="text-[10px] text-gray-400 font-medium uppercase tracking-wider">Business Manager</p>
          </div>
        </div>
      </div>
      ...
      <div className="px-5 py-4 border-t border-gray-100">
        ...
        <p className="text-xs text-gray-400">{user?.email || 'admin@cofeeloot.com'}</p>
      </div>
    </aside>
  );
};
